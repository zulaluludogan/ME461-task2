...
while run:
    ...
    frame = draw_landmarks.draw_landmarks_on_hand(frame, mp_controller.hand_result)
    ...
    if index:
            ...
            index_posy = index[1]*window_height 
            print("Index pos:",index_posy)
            # bats[player_id].y = index_posy
            # bats[player_id].draw(win)

            if index_posy < window_height /3:
                bats[player_id].move('up')

            # ‘S’ to move player’s bat downwards
            if index_posy  >= window_height *2/3:
